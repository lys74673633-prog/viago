...
export function GradeGapChart({ analysis }: GradeGapChartProps) {
  ...
  const plotMax = chart.yMax ?? 9;
  const allY = [...chart.currentLine, ...chart.targetLine, ...chart.realisticLine];
  const plotMin = Math.max(1, Math.min(...allY) - 0.3);

  const xAt = (i: number) =>
    pad.l + (chart.labels.length <= 1 ? 0 : (i / (chart.labels.length - 1)) * innerW);
  const yAt = (grade: number) =>
    pad.t + ((grade - plotMin) / Math.max(0.01, plotMax - plotMin)) * innerH;
    ...
    <div className="rounded-2xl bg-white/90 p-4 ring-1 ring-line md:p-5">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          <h3 className="text-sm font-semibold text-ink">
            등급 갭 · 상승 필요량
            <span className="ml-2 text-[11px] font-medium text-teal-deep">{scaleLabel}</span>
          </h3>
      ...
      <svg viewBox={`0 0 ${w} ${h}`} className="mt-3 w-full" role="img" aria-label="등급 추이 그래프">
        {[0, 0.25, 0.5, 0.75, 1].map((t) => {
          ...
          const grade = plotMin + t * (plotMax - plotMin);
          return (
            <g key={t}>
      ...
      <ul className="mt-2 flex flex-wrap gap-4 text-[11px] text-ink-soft">
        <li>회색 점선: 현재 유지</li>
        <li className="text-[#059669]">초록: 현실적 개선 경로</li>
        <li className="text-[#e11d48]">빨강: 목표 추정 등급</li>
      </ul>
